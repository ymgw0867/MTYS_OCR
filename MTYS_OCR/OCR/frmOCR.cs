...
namespace MTYS_OCR.OCR
{
    public partial class frmOCR : Form
    {
        ...
        ///----------------------------------------------------------------
        /// <summary>
        ///     帳票認識ライブラリ V8.0.3 による認識処理実行　2014/09/20
        /// </summary>
        ///----------------------------------------------------------------
        private void wrhs803LibOCR()
        {
            // ファイル名のタイムスタンプを設定
            ...
            try
            {
                ...
                foreach (string files in System.IO.Directory.GetFiles(Properties.Settings.Default.trayPath, "*.tif"))
                {
                    ...
                    if (ret > 0)
                    {
                        ...
                        outimage.Append(Properties.Settings.Default.winOutPath + System.IO.Path.GetFileName(files));

                        // 認識結果出力テキストファイル
                        StringBuilder outtext = new StringBuilder(256);
                        outtext.Append(Properties.Settings.Default.winOutPath + System.IO.Path.GetFileNameWithoutExtension(files) + ".csv");

                        // 認識結果 構造体
                        FormRecog.FORM_RECOG_DATA dt = new FormRecog.FORM_RECOG_DATA();

                        // 認識処理を開始
                        ret = FormRecog.OcrFormRecogStart(Properties.Settings.Default.wrHands_Job_Image,
                                                              files,
                                                              outimage,
                                                              outtext,
                                                              ref dt,
                        ...
                        if (ret > 0)
                        {
                            ...
                            // READフォルダ → DATAフォルダ
                            string inCsvFile = Properties.Settings.Default.winOutPath +
                                               Properties.Settings.Default.winReaderOutFile;
        ...
        private void wrhOutFileRename(string readFilePath, string newFnm)
        {
            ...
            // 画像ファイルをリネーム
            System.IO.File.Move(Properties.Settings.Default.winOutPath + "WRH00001.tif", newFnm + ".tif");
        }
